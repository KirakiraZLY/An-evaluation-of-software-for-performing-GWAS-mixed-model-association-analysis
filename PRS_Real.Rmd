...
fam1 = cbind(fam1, anestr = "AsianSWC")
fam2 = cbind(fam2, anestr = "Chinese")
fam3 = cbind(fam3, anestr = "Black")
fam4 = cbind(fam4, anestr = "Mixed")
fam5 = cbind(fam5, anestr = "White")

fam_total <- rbind(fam1, fam2, fam3, fam4, fam5)
fam_total <- select(fam_total, c(1,2,6,7))
```
...
covar <- read_table2("covar_PC.covars", col_names = TRUE)
covar$Sex <- as.factor(covar$Sex)
covar$Ancestry <- as.factor(fam_total$anestr)
covar <- select(covar, c(1:4))
levels(covar$Sex) <- c("Male","Female")
# names(PRS)[3] <- "Phenotype"
dat <- merge(merge(PRS, Pheno, by = c("FID","IID")),covar, by = c("FID","IID"))
...
ggplot(dat, aes(x = PRS, y = Phenotype, col = Ancestry)) +
  ...
  xlab("Polygenic Risk Score in Asian") +
  ylab("Urate") +
  labs(title = "PRSice2, Regenie, Asian, Urate") +
  NULL
...
ggplot(dat, aes(x = PRS, col = Ancestry)) +
  ...
  xlab("PRS score") +
  ylab("Density") +
  labs(title = "PRS distribution stratified by Sex, BY PRSice2, Regenie, Urate") +
  NULL
```
